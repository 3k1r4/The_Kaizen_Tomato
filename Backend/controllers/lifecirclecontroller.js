...
exports.lifeactive = async (req,res) => {
    ...
    if(!email){
        ...
        try{
            ...
            {
                const [deletestep] = await databasePool.execute("delete from lifeCircle where active=1 and email=?",[email])
                const [resetstep]= await databasePool.query("select id from lifeCircle where active=0 and email=? order by step",[email] );
                let c=0;
...
exports.lifeget = async (req,res) => {
    ...
    if(!email){
        ...
        try{
            const active = 0;
            const [lifecircle] = await databasePool.query("SELECT * FROM lifeCircle WHERE email=? and active=? or active=NULL ORDER BY step",[email,active]);
...
exports.lifereset = async (req,res) => {
    ...
    try{
     ...
     if(!email){
        ...
        var insertvalue = [
            ["Pomodoro", 25, "tomato", email, 1, 1],
            ["Pausa", 5, "break", email, 2, 1],
            ["Pomodoro", 25, "tomato", email, 3, 1],
            ["Pausa", 5, "break", email, 4, 1],
            ["Pomodoro", 25, "tomato", email, 5, 1],
            ["Pausa Lunga", 15, "long break", email, 6, 1]
        ];
...
exports.getnextstep = async (req, res) => {
    try {
        ...
        const [maxstep] = await databasePool.query("SELECT MAX(step) as bigger FROM lifeCircle where email = ? and active = 1",[email]);
        ...
        const [lifeCircle] = await databasePool.query("Select * From lifeCircle where email=? and step=? and active=1", [email, stepprox])
...
exports.createlifecircle = async (req, res) => {
    ...
    const [numero] = await databasePool.query("SELECT COUNT(*) AS count FROM lifeCircle WHERE email=? and active=0",[email]);
    ...
    if (!name || !duration || !lifeType || !email) {
        ...
        try {
            const active = 0;
            const [lifeCircle] = await databasePool.query("INSERT INTO lifeCircle(name,duration,lifeType,email,descrizione,step,active) VALUES(?,?,?,?,?,?,?)", [name,duration,lifeType,email, descrizione,count, active])
            return res.status(200).json({
    ...
    try {
        ...
        if (step == stepmax) {
            ...
            const [time] = await databasePool.execute(
                "SELECT duration, lifeType FROM lifecircle WHERE active=1 and step = ? AND email = ?",
